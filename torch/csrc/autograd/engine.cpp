...
struct FunctionTask {
    ...
    FunctionTask(GraphTask* base, std::shared_ptr<Function> fn, InputBuffer inputs)
        : base(base), fn(fn), inputs(std::move(inputs)) {}
};
...
struct GraphTask {
    ...
    GraphTask(bool keep_graph, const Engine::pre_callback_map& pre_callbacks,
              const Engine::post_callback_map& post_callbacks)
        : exception(),
          has_error(false),
          outstanding_tasks(0),
...
auto Engine::thread_main(GraphTask* graph_task) -> void {
    ...
    while (!graph_task || graph_task->outstanding_tasks > 0) {
        ...
        // 这种情况下很简单，将 graph_task.outstanding_task -1 ,如果 为 0, 就 notify_all 一下，
        // 告诉其它阻塞的伙计们，老子干完活了。 Task from a non-worker thread. Easy case.
...
auto Engine::compute_dependencies(function_queue queue, GraphTask& task) -> void {
    ...
    while (queue.size() > 0) {
        ...
        for (auto& next_fn_pair : fn->next_functions) {
            ...
            // 如果没有见过 next_ptr, 就把 next_ptr 加到 queue 中。 全部遍历一遍， 所有反向传导函数的 dependencies
            // 个数都会保存在 base.dependencies 中！！！！
            if (seen.count(next_ptr) == 0) {
                seen.insert(next_ptr);
                queue.push_back(next_ptr);
            }
...
struct ClearCallbacks {
    ClearCallbacks(std::vector<std::function<void()>>& callbacks, std::mutex& callbacks_lock)
        : callbacks(callbacks), callbacks_lock(callbacks_lock) {
        clear();
    }
...
auto Engine::execute(const function_list& input_roots, const variable_list& inputs, bool keep_graph,
                     const pre_callback_map& pre_callbacks, const post_callback_map& post_callbacks) -> void {
    // 只需要执行一次的函数， 而且线程安全！！！！！，
    ...
    */
    ...
    //这边的意思是，硬生生的搞出一个 graph 的 root 出来（Function）， 将 inputs(head-gradients) 作为这个 roots 的
    //输出，将 inputs_roots 作为 这个 root 的 next_functions!!!!!!!
    // inputs 为 head-gradients.   graph_root 即 反向传导计算图的 root. 反向传导计算图的每个节点都是一个 grad_fn
    // 反向传导计算图的 leaf-node 为 AccumulateGrad Function. 在创建 leaf-variable 时候就创建了这个 grad-fn
    ...
    for (auto entry : input_roots) {
        if (entry.first->is_executable) {
            ...
            // read_queue 返回一个 ReadyQueue， -1 为 device_id（代表CPU）, InputBuffer 为0 说明 root ,因为是 root，
            // 没有输入，只有输出 把这个 造好的 graph root 的 FunctionTask 放到 CPU 的 ReadyQueue 中
    ...
    if (!graph_task.has_any_work) {
        throw std::runtime_error("there are no graph nodes that require computing gradients");
    }
...
auto Engine::ready_queue(int device) -> ReadyQueue& { return *ready_queues.at(device + 1); }
